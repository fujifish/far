...
class DeployCmd extends Cmd {
  ...
  async _pushDocker(data) {
    return new Promise(async (resolve, reject) => {
      ...
      let image = await docker.getImage(data.repoUri);
  ...
  async _buildDocker() {
    return new Promise(async (resolve, reject) => {
      let built = false;
      let tagged = false;
      let repoUri = await this._getRepository(true);
      ...
      try {
        ...
        let pack = tar.pack(".", {
          ignore: name => {
            ...
            if (ignored || name.endsWith("caprizaConfig.json") || name.endsWith("systemConfig.json")) {
        ...
        rl.on("close", () => {
          if (built && tagged) {
            resolve({ repoTagname, repoUri, tagvalue });
          } else {
    ...
    if (destination) {
      ...
      let logDestination = destination.replace("*", this.config.region);
        await this.cwl
          ...
          .promise();
        terminal.progressEnd();
        terminal.info(`Registered CloudWatchLogs destination: ${logDestination}`);
    }
  ...
  async _checkHasNatGateway() {
    let vpcs = await this.ec2
      .describeVpcs({
        Filters: [
          {
            Name: "tag:Name",
    ...
    if (vpcs.Vpcs.length > 0) {
      ...
      if (result.NatGateways.length > 0) {
        terminal.info("Found NAT Gateway configured");
        return true;
      }
    } else {
      terminal.info("No Customer VPC found");
    }
    
    terminal.info("No NAT Gateway found");
    return false;
  }
  ...
  async _ensureService(taskDefinition) {
    ...
    const hasNatGateway = await this._checkHasNatGateway();
    this.config.assignPublicIp = !hasNatGateway;
