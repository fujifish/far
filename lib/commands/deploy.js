...
class DeployCmd extends Cmd {
  ...
  async _buildDocker() {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        // force a tag name for the build
        this.config.dockerBuild.t = repoTagname;

        // kickoff the build
        let response = await docker.buildImage(pack.pipe(zlib.createGzip()), this.config.dockerBuild);
  ...
  async _ensureService(taskDefinition) {
    ...
    let assignPublicIp = (this.config.assignPublicIp === true || this.config.subnets === "awsDefaultVpc") ? "ENABLED" : "DISABLED";
