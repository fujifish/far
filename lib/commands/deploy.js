...
class DeployCmd extends Cmd {
  ...
  async _registerTaskDefinition(image) {
    ...
    let logDestination =
      this.config.logDestination !== undefined ? this.config.logDestination : this.awsConfig.log_destination;
    if (logDestination) {
      await this.cwl
        ...
        containerDefinitions: [
          {
            ...
            logConfiguration: {
              ...
              options: {
                ...
                "awslogs-datetime-format": this.config.logDatetimeFormat || "%Y-%m-%dT%H:%M:%S.%LZ"
              }
            }
          }
        ],
  ...
  async _getSubnets() {
    ...
    terminal.progressStart("Looking up subnets with tag:farSubnet=true");
    let result = await this.ec2
      .describeSubnets({
        Filters: [
          {
    ...
    terminal.progressInfo("No subnets with tag:farSubnet=true found");
    terminal.progressStart("Retrieving default subnets");
