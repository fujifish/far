...
class DeployCmd extends Cmd {
  ...
  async _updateService(image) {
    ...
    if (cluster.status !== "ACTIVE") {
      terminal.exit(`cluster ${cluster.clusterName} is ${cluster.status}`);
    }
  ...
  async _buildDocker() {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        let result = await this.ecr.describeImages({
      ...
      let dockerflile = this.config.dockerfile || "Dockerfile";
      let docker = new Docker();
      try {
        terminal.log(`Building docker image from ${dockerflile}`);
