    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (this.config.creds) {
        try {
          ...
          terminal.info(`using credentials from file ${this.config.creds}. apiKey: ${asCreds.apiKey}`);
        } catch(e) {
      ...
      try {
        ...
        terminal.info(`obtaining credentials for connectorId ${connectorId} from ${caprizaConfig.apiUrl}`);
        if (this.config.creds) {
          try {
