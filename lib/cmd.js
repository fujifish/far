...
class Cmd {
  ...
  async getCallerIdentity() {
    return new Promise(async (resolve, reject) => {
      // get the connector details from the caprizaConfig.json
      let file;
      try {
        file = fs.readFileSync(`./resources/${this.config.env}/caprizaConfig.json`)
      } catch(e) {
        try {
          file = fs.readFileSync("./resources/caprizaConfig.json")
        } catch(e) {
          reject(new Error(`could not find caprizaConfig.json at "resources/${this.config.env}/caprizaConfig.json" or "resources/caprizaConfig.json"`));
          return;
        }
      }

      try {
        let caprizaConfig = JSON.parse(file);
        ...
        if (this.config.creds) {
          try {
            ...
            let asCreds = JSON.parse(fs.readFileSync(asCredsFile));
            ...
            reject(new Error(`failed to get deployment credentials: ${e.message}`));
          }
        } else {
